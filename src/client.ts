...
export class MitIDClient {
	...
	private createFlowValueProofData(): Buffer {
		const hashedBsc = createHash("sha256")
			...
			.digest("hex");
		const b64Header = Buffer.from(this.referenceTextHeader, "utf-8").toString(
			"base64",
		);
	...
	async authenticateWithApp(onStatus?: StatusCallback): Promise<void> {
		...
		if (!proveResp.ok) throw new Error(`SRP prove failed: ${proveResp.status}`);
	...
	async finalize(): Promise<string> {
		if (!this.finalizationSessionId) throw new Error("No finalization session");
