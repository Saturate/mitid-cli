...
export class CustomSRP {
	...
	private computeLittleS(): bigint {
		...
		gBytes = Buffer.concat([
			Buffer.alloc(nBytes.length - gBytes.length),
			gBytes,
		]);

		// DEVIATION: N is hashed as its decimal string representation
		const hash = sha256(
			Buffer.concat([Buffer.from(N.toString(10), "utf-8"), gBytes]),
		);
	...
	private computeSessionKey(): bigint {
		...
		const base =
			(((this.B - modPow(g, this.hashedPassword, N) * s) % N) + N) % N;
